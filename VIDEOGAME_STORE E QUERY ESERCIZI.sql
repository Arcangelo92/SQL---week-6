...
FROM
    videogioco
WHERE
    AnnoDistribuzione LIKE '2020%';

-- NON AVENDO DATI PER SODDISFARE LA CONSEGNA -> 
-- Seleziona i titoli dei videogiochi disponibili nei settori 1 o 3 del negozio 5
-- Ho estrapolato l'indirizzo dello store dove è venduto un determinato videogioco
SELECT 
    IndirizzoFisico
FROM
    store
        JOIN
    videogioco ON store.CodiceStore = videogioco.CodiceStore
WHERE
    videogioco.CodiceStore = '5'

-- NON AVENDO DATI PER SODDISFARE LA CONSEGNA -> 
-- Seleziona i negozi con più di 20 copie disponibili di almeno un videogioco.
-- Ho visualizzato il nome del videogioco 
-- rispetto alla collocazione in determinati store da 3 a 9 e al costo di acquisto minore di 50€
SELECT 
    Titolo AS Nome
FROM
    videogioco AS game
        JOIN
    store ON store.CodiceStore = game.CodiceStore
WHERE
    store.CodiceStore BETWEEN '3' AND '9'
        AND game.CostoAcquisto < '50';
